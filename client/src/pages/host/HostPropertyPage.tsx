...
import { ArrowLeft, ImageIcon, Settings, DollarSign, Calendar } from "lucide-react";
          ...
          <TabsContent value="settings">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input value={editForm.icalUrl} onChange={e => setEditForm({...editForm, icalUrl: e.target.value})} placeholder="https://www.airbnb.com/calendar/ical/..." className="h-9 text-xs font-mono" />
                <p className="text-[10px] text-muted-foreground mt-1">Airbnb → Calendar → Availability settings → Export calendar → Copy link</p>
              </div>
              <div className="flex gap-2 pt-2"><Button onClick={handleSave} disabled={isSaving}>{isSaving ? "Saving..." : "Save Settings"}</Button></div>
            </div>
          </TabsContent>
        </Tabs>
      </main>
